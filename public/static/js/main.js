...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Track whether location has been obtained
  let locationObtained = false;

  // Add click handler to show notification when clicking disabled Generate Plan button
  if (Sub_btn) {
    Sub_btn.addEventListener("click", function (e) {
      if (!locationObtained) {
        e.preventDefault();
        e.stopPropagation();
        alert("Please click 'Use Current Location' button first to enable plan generation.");
        return false;
      }
    });
  }
  ...
  if (button) {
    button.addEventListener("click", function () {
      // Keep button disabled during location fetch
      if (Sub_btn) Sub_btn.disabled = true;
      ...
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        fetch("send-coordinates/", {
          ...
          .then(data => {
            ...
            // Mark location as obtained and enable button
            locationObtained = true;
            if (Sub_btn) Sub_btn.disabled = false;
          })
          .catch(error => {
        ...
        // Keep button disabled if location access denied
        if (Sub_btn) Sub_btn.disabled = true;
      });
    });
  }
